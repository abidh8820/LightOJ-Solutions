...
struct Dinic {
    ...
    Dinic(int n, int s, int t) : n(n), s(s), t(t) {
        lvl.resize(n + 100), ptr.resize(n + 100);
    }
...
int main() {
    ...
    while (t--) {
        ...
        Dinic graph(10000, 150, 151);

        for (int i = 0; i < n; i++) {
            graph.add_edge(150, i, 1);
        }

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (b[j] % a[i] == 0) graph.add_edge(i, j + 1000, 1);
            }
        }

        for (int i = 0; i < m; i++) graph.add_edge(i + 1000, 151, 1);

        cout << "Case " << ++C << ": " << graph.flow() << endl;
    }
}
